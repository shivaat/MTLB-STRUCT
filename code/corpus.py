...
class Corpus:
    ...
    def __init__(self, path):
        ...
        self.devSents, self.testSents = [], []
        self.devMweNum, self.testMweNum = 0, 0
        if devCupt is not None:    # 'modified'
            self.devSents, self.devMweNum = Corpus.readCuptFile(devCupt)
        if testCupt is not None:    # 'modified'
            self.testSents, self.testMweNum = Corpus.readCuptFile(testCupt)
        if blindTestCupt is not None:    # 'modified'
            self.blindTestSents = Corpus.readBlindTestFile(blindTestCupt)
        else:
            self.blindTestSents = None
        ...
        with open(cuptFile) as corpusFile:
            ...
            for line in lines:
                ...
                if line.startswith('# sent_id:'):
                    ...
                    sentenceText = line.split('# text =')[1].strip()    # 'modified'
                    continue
                ...
                if not line.startswith('#'):
                    ...
                    if len(lineParts) != 11 or '-' in lineParts[0]:     # 'modified' to 11
                        continue
                    morpho = ''
                    if lineParts[5] != '_':
                        morpho = lineParts[5].split('|')
                    if lineParts[6] != '_' and lineParts[6] != '-':     # 'added' 
                        token = Token(lineParts[0], lineParts[1], lemma=lineParts[2],   ##  'removed' .lower() from lineParts[1]
                        ...
                        # These cases of 8.1 or 22.1 are tokens 8 and 22 repeated in the paprsed sentence!!!
                        token = Token(lineParts[0], lineParts[1], lemma=lineParts[2],   ## 'removed' .lower() from lineParts[1]
                                      abstractPosTag=lineParts[3], morphologicalInfo=morpho,
                                      dependencyLabel=lineParts[7])
                    if lineParts[4] != '_':
        ...
        with open(cuptFile) as corpusFile:
            ...
            for line in lines:
                ...
                if line.startswith('# sent_id:'):
                    ...
                    sentenceText = line.split('# text =')[1].strip()    # ADDED
                    continue
                elif line.startswith('#'):
                    continue
                ...
                if not line.startswith('#'):
                    ...
                    if len(lineParts) != 11 or '-' in lineParts[0]:     # CHANGED it to 11
                        continue
                    morpho = ''
                    if lineParts[5] != '_':
                        morpho = lineParts[5].split('|')
                    if lineParts[6] != '_' and lineParts[6] != '-':     # 'added'
                        ...
                        print('Blind test, No dependency!!!',lineParts)
                        token = Token(lineParts[0], lineParts[1], lemma=lineParts[2],   ## 'removed' .lower() from lineParts[1]
                                      abstractPosTag=lineParts[3], morphologicalInfo=morpho,
                                      dependencyLabel=lineParts[7])
                    if lineParts[4] != '_':
...
class Token:
    ...
    def __init__(self, position, txt, lemma='', posTag='', abstractPosTag='', morphologicalInfo=[], dependencyParent=-1,
                 dependencyLabel=''):
        try:    # added only for one case of line number 24.1 in he English data
